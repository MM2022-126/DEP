...
public class Server {
	...
	private static int port = 12345;  // port number to be used
		...
		public void run() {
			while (true) {
				try {
					...
					String U_Name = new DataInputStream(clientSocket.getInputStream()).readUTF(); // this will receive the username sent from client register view
					DataOutputStream cOutStream = new DataOutputStream(clientSocket.getOutputStream()); // create an output stream for client
					if (activeUserSet != null && activeUserSet.contains(U_Name)) { // if username is in use then we need to prompt user to enter new name
						cOutStream.writeUTF("Username already taken");
					} else {
						allUsersList.put(U_Name, clientSocket); // add new user to allUserList and activeUserSet
						activeUserSet.add(U_Name);
						cOutStream.writeUTF(""); // clear the existing message
						activeDLM.addElement(U_Name); // add this user to the active user JList
						if (!allDLM.contains(U_Name)) // if username taken previously then don't add to allUser JList otherwise add it
							allDLM.addElement(U_Name);
						...
						serverMessageBoard.append("Client " + U_Name + " Connected...\n"); // print message on server that new client has been connected.
						new MsgRead(clientSocket, U_Name).start(); // create a thread to read messages
	...
	class MsgRead extends Thread { // this class reads the messages coming from client and take appropriate actions
		Socket skt;
		String Id;
		private MsgRead(Socket skt, String U_Name) { // socket and username will be provided by client
			this.skt = skt;
			this.Id = U_Name;
		}
		...
		public void run() {
			while (allUserNameList != null && !allUsersList.isEmpty()) {  // if allUserList is not empty then proceed further
				try {
					String message = new DataInputStream(skt.getInputStream()).readUTF(); // read message from client
